#include "StdUIBatch.h"

StdUIBatch::StdUIBatch(EfficientLookup<std::shared_ptr<IBatchable>> batchables)
    : BatchBase(std::move(batchables))
{
}

void StdUIBatch::BuildPSOs(ID3D12Device* device)
{
    BuildStdUiPSO(device, mPSOs["std_ui"]);
}

void StdUIBatch::BuildStdUiPSO(
...
std::vector<D3D12_INPUT_ELEMENT_DESC> StdUIBatch::StdUiInputLayout()
{
...
void StdUIBatch::CompileStdShaders()
{
...
void StdUIBatch::BuildStdRootSignature(ID3D12Device& device)
{
...
void StdUIBatch::SetRootSignature(ID3D12GraphicsCommandList* cmdList)
{
    cmdList->SetGraphicsRootSignature(mRootSignatures["std_ui"].Get());
}

void StdUIBatch::SetPSO(ID3D12GraphicsCommandList* cmdList)
{
